  ...
  return (
    <div className="max-w-6xl space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <Link href="/admin/visas-blogs/new" className="px-3 py-1 border border-blue-600/30 text-blue-600 text-sm font-medium rounded hover:bg-blue-50 transition-colors">
          Add New Post
        </Link>
      </div>
              ...
              {mockItems.filter(p => activeTab === 'All' || p.status === activeTab).map((item) => (
                <tr key={item.id} className="hover:bg-gray-50 group transition-colors">
                  ...
                  <td className="px-4 py-3 align-top">
                    <Link href={`/admin/visas-blogs/new`} className={`font-semibold text-blue-700 hover:text-blue-600 transition-colors`}>
                      {item.title}
                    </Link>
                    ...
                    <div className="h-5 flex items-center gap-2 mt-1 opacity-0 group-hover:opacity-100 transition-opacity text-[12px] pt-1">
                      <Link href={`/admin/visas-blogs/new`} className="text-blue-600 hover:text-blue-800 hover:underline">Edit</Link>
                      <span className="text-gray-300">|</span>
