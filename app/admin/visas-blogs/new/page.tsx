...
import { ArrowLeft, Upload, Settings, Calendar, Italic, Bold, List, Image as ImageIcon } from 'lucide-react';

export default function EditVisaBlog() {
    ...
    <div className="max-w-7xl">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          <Link href="/admin/visas-blogs" className="w-8 h-8 flex items-center justify-center rounded border border-gray-300 bg-white hover:bg-gray-50 transition-colors tooltip" aria-label="Back">
            <ArrowLeft className="w-4 h-4 text-gray-600" />
          </Link>
          <h1 className="text-2xl font-normal text-[#1d2327]">Add New Post (Visa/Blog)</h1>
        </div>
      </div>
        ...
        <div className="flex-1 space-y-6">
          ...
          {title && (
            <div className="text-[13px] text-gray-600 px-2 -mt-4">
              <strong>Permalink:</strong> https://maduratravel.com/post/<span className="text-blue-600 bg-amber-50 px-1">{title.toLowerCase().replace(/ /g, '-')}</span>
            </div>
          )}
          ...
          <div className="bg-white border border-gray-200 shadow-sm min-h-[500px] flex flex-col">
            ...
            <textarea 
              ...
              placeholder="Write the content for your blog or visa requirements here..."
            ></textarea>
          </div>
            ...
            <div className="p-4">
              <div className="max-h-40 overflow-y-auto space-y-2 text-[13px]">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" className="text-blue-600 border-gray-300 rounded-sm" /> Travel Blog
                </label>
                <label className="flex items-center gap-2 cursor-pointer ml-4">
                  <input type="checkbox" className="text-blue-600 border-gray-300 rounded-sm" /> Tips & Tricks
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" className="text-blue-600 border-gray-300 rounded-sm" /> Visa Information
                </label>
                <label className="flex items-center gap-2 cursor-pointer ml-4">
                  <input type="checkbox" className="text-blue-600 border-gray-300 rounded-sm" /> Schengen Area
                </label>
              </div>
