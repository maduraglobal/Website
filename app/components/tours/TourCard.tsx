...
export default function TourCard({ tour, destinationSlug }: TourCardProps) {
  ...
  return (
    ...
    >
      {/* Target Image Area */}
      <div className="relative w-full md:w-[350px] h-[250px] md:h-auto overflow-hidden shrink-0">
        <motion.img
          initial={false}
      ...
      <div className="flex-1 p-5 md:p-6 flex flex-col">
        <div className="flex flex-col md:flex-row justify-between gap-4 flex-1">
          ...
          <div className="flex-1 space-y-3">
            ...
            <h3 className="text-xl md:text-2xl font-black text-[#171717] font-poppins leading-tight group-hover:text-[#191974] transition-colors">
              {tour.title}
            </h3>

            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-gray-600 font-medium">
              <span className="flex items-center gap-1.5"><Clock className="w-4 h-4 text-gray-400" /> {tour.duration_days || 6} Days</span>
              <span className="w-1 h-1 rounded-full bg-gray-300" />
              <span className="flex items-center gap-1.5"><MapPin className="w-4 h-4 text-gray-400" /> {tour.cities_count || 4} Cities</span>
              <span className="w-1 h-1 rounded-full bg-gray-300" />
              <span className="flex items-center gap-1.5"><Calendar className="w-4 h-4 text-gray-400" /> Sep - Dec Dates</span>
            </div>
