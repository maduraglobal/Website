  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sticky top-24">
      ...
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-100">
        <h3 className="text-lg font-bold text-[#191974] flex items-center gap-2">
          <SlidersHorizontal className="w-5 h-5 text-[#ee2229]" /> Filters
        </h3>
        <button className="text-sm font-medium text-[#ee2229] hover:underline">Clear All</button>
      </div>
      ...
      <div className="relative mb-6">
        <input 
          ...
          className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#191974]/20 focus:border-[#191974] transition-all"
        />
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-3">
          <label className="text-sm font-bold text-[#171717] block">Travel Dates</label>
          <div className="relative">
            <input 
              type="month" 
              className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-700 focus:outline-none focus:border-[#191974]"
            />
        ...
        <div className="border-t border-gray-100 pt-6">
          ...
          >
            <span className="text-sm font-bold text-[#171717]">Cities</span>
            ...
            <div className="mt-4 space-y-3">
              {['Delhi', 'Agra', 'Jaipur', 'Shimla', 'Manali'].map((city) => (
                <label key={city} className="flex items-center gap-3 cursor-pointer group">
                  ...
                  <span className="text-sm text-gray-600 group-hover:text-[#191974] transition-colors">{city}</span>
                </label>
              ))}
            </div>
          )}
        ...
        <div className="border-t border-gray-100 pt-6">
          <button 
            ...
            className="flex items-center justify-between w-full text-left"
          >
            <span className="text-sm font-bold text-[#171717]">Tour Duration</span>
          ...
          {isExpanded.duration && (
            <div className="mt-4 flex flex-wrap gap-2">
              {['1-3 Days', '4-6 Days', '7-10 Days', '11+ Days'].map((duration) => (
                <button key={duration} className="px-3 py-2 text-xs font-medium border border-gray-200 text-gray-600 rounded-lg hover:border-[#191974] hover:bg-[#191974]/5 transition-all focus:ring-2 focus:ring-[#191974]/20">
                  {duration}
                </button>
              ))}
            </div>
        ...
        <div className="border-t border-gray-100 pt-6">
          <button 
            ...
            className="flex items-center justify-between w-full text-left"
          >
            <span className="text-sm font-bold text-[#171717]">Specialty Tour</span>
            ...
            <div className="mt-4 space-y-3">
              {['Family', 'Honeymoon', 'Group Tour', 'Adventure', 'Luxury'].map((specialty) => (
                <label key={specialty} className="flex items-center gap-3 cursor-pointer group">
                  ...
                  <span className="text-sm text-gray-600 group-hover:text-[#191974] transition-colors">{specialty}</span>
                </label>
              ))}
            </div>
          )}
